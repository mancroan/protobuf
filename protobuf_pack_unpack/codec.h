...
inline google::protobuf::Message* createMessage2(const std::string& type_name, google::protobuf::compiler::Importer &importer, google::protobuf::DynamicMessageFactory &factory)
{
	google::protobuf::Message* message = NULL;
	const google::protobuf::Descriptor* descriptor = importer.pool()->FindMessageTypeByName(type_name);
	if (descriptor)
	{
...
inline google::protobuf::Message* decode2(const std::string& buf,  google::protobuf::compiler::Importer &importer, google::protobuf::DynamicMessageFactory &factory)
{
	google::protobuf::Message* result = NULL;
	int32_t len = static_cast<int32_t>(buf.size());
	if (len >= 10)
	{
		...
		if (checkSum == expectedCheckSum)
		{
			...
			{
				std::string typeName(buf.begin() + kHeaderLen, buf.begin() + kHeaderLen + nameLen - 1);
				google::protobuf::Message* message = createMessage2(typeName, importer, factory);
				if (message)
				{
					const char* data = buf.c_str() + kHeaderLen + nameLen;
					int32_t dataLen = len - nameLen - 2*kHeaderLen;
					std::string ret;
					ret.append(data, dataLen);
